...
int AESencryption(string input)
{
    freopen(input.c_str(),"r",stdin);
    char message[1024];

    cout << "the message to encrypt: ";
    cin.getline(message, sizeof(message));
    /*if (infil.is_open())
    {
        string line;
        getline(infil, line); // The first line of file should be the key
        infil.close();

        std::stringstream ss(line);
        ss.getline(message, sizeof(message));
    }
...
int AESdecryption(string message)
{
    string msgstr;
    ifstream infile;
...
string sha512(string s)
{
    ...
    //cout<<"After add size: "<<originalMessage<<endl;
